...
Ant.prototype.takeFood = function (food) {
    if (this.isFood(food)) { // If food exists
...
Ant.prototype.atNest = function () {
    // Check each ant in cell, if any are of type ANT_TYPE.nest then on nest
...
Ant.prototype.seeNest = function () {
    // Checks each ant in view, if any of type ANT_TYPE.nest, then can see nest
...
Ant.prototype.useFood = function () {
    ...
    if (this.isFood(food)) // check the food still exists i.e. not already eaten
        this.health += this.takeFood(food) * FOOD_HEALTH_RATIO;
    else // Food is not there/cannot be carried
        this.target = void(0); // no target
};
/**
...
Ant.prototype.wonder = function () {
    ...
    CoM = {
        x: Mxy.x / M,
        y: Mxy.y / M
    }; // The mean coordinate of all the 
...
Ant.prototype.die = function () {
    ...
    ANTS_LIST.splice(index, 1); // remove from ANTS_LIST
    var index = this.species.ants.indexOf(this); // remove from species.ants
