...
function drawGrid(ctx) {
    for (var x = 0; x < GRID_SIZE.width + 1; x += 1) // Draw lines parallel to the x-axis 
        drawLine(ctx, {
            x: x * CELL_SIZE.width + CANVAS_OFFSET.x,
            y: CANVAS_OFFSET.y
        }, {
            x: x * CELL_SIZE.width + CANVAS_OFFSET.x,
            y: GRID_SIZE.height * CELL_SIZE.height + CANVAS_OFFSET.y
        }, GRID_COLOUR, GRID_LINE_WIDTH);

    for (var y = 0; y < GRID_SIZE.height + 1; y += 1) // Draw lines parallel to the y-axis 
        drawLine(ctx, {
            x: CANVAS_OFFSET.x,
            y: y * CELL_SIZE.height + CANVAS_OFFSET.y
        }, {
            x: CELL_SIZE.height * GRID_SIZE.height + CANVAS_OFFSET.x,
            y: y * CELL_SIZE.height + CANVAS_OFFSET.y
        }, GRID_COLOUR, GRID_LINE_WIDTH);
}
...
function drawBackground(ctx) {
    drawRect(ctx, CANVAS_OFFSET, {
        width: GRID_SIZE.width * CELL_SIZE.width,
        height: GRID_SIZE.height * CELL_SIZE.height
    }, BACKGROUND_COLOUR);
}
 ...
 */
function zoom(level) {
    if (CELL_SIZE.width + level > MIN_ZOOM && CELL_SIZE.width + level < MAX_ZOOM &&
        CELL_SIZE.height + level > MIN_ZOOM && CELL_SIZE.height + level < MAX_ZOOM) {
