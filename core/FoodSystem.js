...
var FoodSystem = function() {
	/**
	 * @property {min : integer, max : integer} this.variation - The minimum and maximum amounts of food a single piece can contain
...
* @param {integer} radius - The maximum distance a piece of food can be from the centre coord
*/
FoodSystem.prototype.addRandFood = function(coord, radius) {
	var affectedCells =  getSector(coord, radius, 0, 2 * Math.PI)	// Get all cells which lie in the circle
	...
	for (var i = 0; i < affectedCells.length; i++) {
		...
		if (amount >= this.variation.min)
			MAP[coordToIndex(affectedCells[i])].food = new Food(this, amount, affectedCells[i]);	// create food and add to map
	}
};
